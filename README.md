# 🗡 Kingshot Swordland Showdown Guide by @engenuity ⚔

**Welcome, Legion! This is our detailed guide to dominate Swordland.** We're splitting into **3 Whale-led Squads** - each whale leads a handful of fish for clear responsibilities. Let's focus on nailing basics and teamwork for now, with advanced strategy happening later...winning will happen naturally as we improve 🏆. Be online 10 mins early, join Discord voice for coordination (English-speakers)

## 📚 **Quick Guide**
- **No Losses:** All troops heal free – fight aggressive!  
- **Pre-battle Preparation:**
   - Recall all marches home. Heal infirmary 100%
   - Activate buffs/gear/heroes, use Howard, Gordon, Quinn, and Amadeus for defense
   - Set defense march presets for 70% infantry/30% cavalry/0% archers (if possible)
   - Join Discord voice (English speakers) 5 minutes before start (commit full 60 mins)
- **In-game Freebies**
   - 5 free initial march speedups (regenerate 1/min, max 30)
   - 1 free initial teleport (regenerate every 10 min --> 5 min w/ Stables, max 1). Plan, don't spam!
   - Initial free healing speedups (regenerate ?/min, max unknown). If you're a whale and get wiped, use the 12-minute healing reset trick 👇🏽
   - Healing Reset: Tap scoreboard banner (top screen) --> "Leave Battlefield" --> free full heal --> rejoin after 12 min cooldown
- **City Burn Tactic:** If a city is destroyed --> garrisoned troops teleport home with city (resets their garrisoned troops back to their city)
- **Counter the Burn Tactic:**   
  - **Troop Dance** to dodge incoming attacks: send city troops to camp next to you --> recall --> teleport away safe. Garrisons stay!
  - Spread out – don't cluster. Garrisoned fish are safer spread between buildings, not next to them. Starve the enemy of teleports ⚡
- **Coordinate:** Discord: "Squad A Garrison Stables" "Squad C Rally Hall" Whales ping/tag 
- **If Behind:** Guerrilla – burn, scoop supplies, under-cellars, flip quick for first-capture bonuses  


## 🏨 **Buildings List & Effects** (Earn Points)
| Building | Effect | Phase | Why Important |
|----------|--------|-------|---------------|
| **Royal Stables** | 50% teleport cooldown reduction (10 min --> 5 min) | 1 | Faster moves for everyone – PRIORITY #1! |
| **Bell Tower** | 50% faster capture time | 1 | Quick flips on contested spots – PRIORITY #2! |
| **Sanctums** (2 total) | Steady Relic Points over time | 1 | Constant scoring – hold both for lead |
| **Abbeys** (4 total) | Moderate steady Relic Points | 1 | Backup scoring – grab extras |
| **Hall of Reformation** | +Attack & Damage Resistance buffs (whole team) | 2 | Makes us stronger – rally early Phase 2! |
| **Swordshrine** (Center) | Highest continuous Relic Points | 2 | Game-changer – contest with rallies Phase 2-3 |
| **Mercenary Camp** | Send auto-mercs to hit enemy buildings | 2 | Pressure without risk – weaken their holds |
| **Under-cellars** (Random waves) | Big burst Relic Points | 3 | Swing games – hunt on minimap (magnifying glass) |
| **Arsenal Supplies** | Free Relic Points drops | All | Scoop EVERY ONE from lost buildings! |

**Win Condition:** Most **Relic Points** after 60 mins (first captures + holds + supplies + under-cellars).

## Game Strategy
### 💭 3 Whale-Led Squads (~8 per Squad from Legions)
- **Squad A** 🐳 Whale1 owns Royal Stables + 1 Sanctum + 2 Abbeys
- **Squad B** 🐳 Whale2 owns Bell Tower + 1 Sanctum + 2 Abbeyslies
- **Squad C** 🐳 Whale3 owns Merc Camp + Hall of Ref, phase 3 Swordshrine rallies
  
### Squad Strategy (everyone)
- Whales lead captures/rallies with 1st march. 2nd/3rd march for destroying nearby enemy cities, defending secondary buildings, & defending squad cities
- Fish join your squad's rallies and garrison/reinforce your squad's buildings first from a short safe distance. 2nd march for defending your squad's cities and collecting supplies
- Garrison/reinforce buildings with the "Tank" troop ratio (70% infantry/30% cavalry/0% archers) when possible
### 🐟 **Basics for Fish (Defenders/Supports)**
**Your Role:** Hold the line! Follow your Whale.
1. **Teleport:** To your squad's safe zone (R4 will mark). Stay just out of sight but near your squad's buildings, don't cluster together
1. **March Speedups:** Use free ones to arrive FAST for garrisons.
1. **Garrison Everything:** After whale captures --> garrison IMMEDIATELY
1. **Reinforce:** ALWAYS join ally garrisons. Stack > solo
1. **Scoop Arsenal Supplies:** Every drop = points. Prioritize supplies capture after garrisons secure
1. **Spare Marches:** Pick up supplies, capture under-cellars, reinforce other squad members

**Never:** Solo attack, cluster near other squad members or buildings, exit buildings that you've reinforced (unless asked)

### 🐳 **Basics for the 3 Whales (Attack Leaders)**
**Your Role:** Capture, rally, burn, disrupt. Use 1st march main, 2nd march for burns/defense
1. **Teleport/March:** Talk to your squad, let them know when you launch rallys or need support
1. **Phase 1:** Rush capture your squad buildings (Stables/Bell/Sanctums/Abbeys). Garrison quick --> clear out invaders --> move to help others
1. **Burns:** 2nd march on enemy cities near you. Force their garrisons home!
1. **Phase 2:** Rally **Hall** first (buffs). Then **Swordshrine** (you START, squad joins). Send Mercs on enemy holds
1. **Phase 3:** Push Swordshrine rallies (first capture gives a big bonus). Hunt under-cellars w/ squad. Rally enemy buildings for supply drops that fish can collect
1. **Rotations:** Healing Reset/Troop Dance to stay strong
1. **City Killers:** You + dolphins = burn squad. Target enemy whales first

## 📊 **Alliance Phases (60 Min Battle)**
| Phase | Time | Squad Focus | All Calls |
|-------|------|-------------|-----------|
| **1: Open Rush** | 0-15 min | Squad A: Stables/Sanctum/Abbeys<br>Bravo: Bell/Sanctum/Abbeys<br>Charlie: Help + Merc | Capture --> Garrison/Reinforce<br>Burn enemies! Scoop supplies |
| **2: Buff & Push** | 15-45 min | Hold bases (split defs)<br>Charlie leads Hall/Swordshrine/Merc | Rally Hall --> Swordshrine<br>Merc pressure. Cross-reinforce |
| **3: Chaos Close** | 45-60 min | Mobile: Under-cellars + Swordshrine | Hunt cellars (minimap)<br>Rally enemies for drops |

**🔥⚔ Squads alive = Victory! Questions --> R4/Discord. Let's CRUSH! ⚔🔥**
